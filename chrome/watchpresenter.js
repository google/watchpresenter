...
function tryRegistration(currentURL) {

    if (!(currentURL.match(/.*#openModal$/))) {
    ...
    setLastRegistrationURL(currentURL);
...
function watchPresenterInit(){
    var currentURL = window.location.href;
    console.log("watchPresenterInit. URL: " + currentURL);
    if (currentURL.match(URLRegexpNoPresent)) {
        console.log("URL match. Checking auth status...");
        getLastRegistrationURL(function (lastRegistrationURL) {
            console.log("Last registration URL: " + lastRegistrationURL);
            if (lastRegistrationURL != removeFragment(currentURL)) {
                tryRegistration(currentURL);
            } else {
        ...
        chrome.extension.sendRequest("hide_page_action");
    }
}
